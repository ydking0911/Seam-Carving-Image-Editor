...
class SeamCarver:
    def __init__(self, in_image, out_height, out_width, protect_mask=[], object_mask=[], seam_callback=None):
        ...
        self.seam_callback = seam_callback
    ...
    def object_removal(self):
        ...
        while len(np.where(self.mask[:, :] > 0)[0]) > 0:
            ...
            if self.seam_callback:
                self.seam_callback(self.out_image, seam_idx)
    ...
    def seams_insertion(self, num_pixel):
        if len(self.protect) > 0:
            ...
            for dummy in range(n):
                ...
                if self.seam_callback:
                    self.seam_callback(self.out_image, seam)
                self.add_seam(seam)
    ...
    # ... (rest of the SeamCarver class remains unchanged)
